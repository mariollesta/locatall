...
def get_top_places(
    ...
    """
    Get the top 5 places by rating (restaurants, cafes, bars) using Google Places API.
    """
    try:
        ...
        for place_type in place_types:
            # Request parameters for each type
            params = {
                "location": f"{lat},{lng}",
                "radius": radius,
                "type": place_type,
            ...
            # Extract places and add to the combined results
            for place in data.get("results", []):
                if place.get("rating") is not None:  # Only include places with a rating
                    combined_results.append({
                        ...
                        "type": place_type,  # Add the type for reference
                        "open_now": place.get("opening_hours", {}).get("open_now", False),
                    })

        # Sort all results by rating (highest first) and take the top 5
        top_places = sorted(combined_results, key=lambda x: x["rating"], reverse=True)[:5]
